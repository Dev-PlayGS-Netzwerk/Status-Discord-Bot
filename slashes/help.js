...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('help') //Name des Befehls - BENENNEN SIE DIE DATEI AUCH UM!!!
        .setDescription(`Sendet das Befehlslistenmenü`) //Beschreibung des Befehls - Sie können ihn ändern :)
...
module.exports.run = async (bot, interaction) => {
    ...
    if (!args[0]) {
        ...
        if (!text.title || !text.description) {
            const helpEmbed = new EmbedBuilder()
                ...
                .setTitle(config.server.name ? config.server.name : interaction.guild.name + " Bot Befehle:")
    ...
    if (bot.commands.has(args[0].toLocaleLowerCase()) || bot.aliases.has(args[0].toLocaleLowerCase())) {
        ...
        const helpEmbed = new EmbedBuilder()
            .setAuthor({ name: config.server.name ? config.server.name : interaction.guild.name, iconURL: icon })
            .setTitle(`${commandName.charAt(0).toUpperCase() + commandName.slice(1)} Befehle:`)
            .setDescription(`
                > **Description:** ${!!command.config.description ? command.config.description : "Ohne Beschreibung"}
