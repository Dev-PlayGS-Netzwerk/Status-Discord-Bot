...
module.exports.config = {
    name: "status", //Name des Befehls - BENENNEN SIE DIE DATEI AUCH UM!!!
    description: "Sendet jetzt die einfache Statusmeldung über den Server", //Beschreibung des Befehls - Sie können ihn ändern :)
    aliases: commands.status.aliases //Aliasnamen des Befehls - setzen Sie sie in config.js
};
    ...
    if (server.type === 'java') {
        util.status(ip1, port1)
            ...
            .catch((error) => {
                ...
                if (warns) console.log(warn(`Fehler bei der Verwendung des Befehls ${module.exports.config.name}! Fehler:\n`) + error);
            });
    } else {
            ...
            .then((result) => {
                ...
                if (text.title === "" || text.description === "") {
                    const serverEmbed = new Discord.EmbedBuilder()
                        .setAuthor({ name: config.server.name ? config.server.name : message.guild.name, iconURL: icon })
                        .setTitle("Serverstatus:")
                        .setDescription(`:white_check_mark: ${maintenceStatus ? ":construction_worker: **Wartungsarbeiten**" : ":white_check_mark: **ONLINE**"}
